...
public interface ProductRepository extends JpaRepository<Product, Long>
{
    ...
    List<Product> findByNameContainingIgnoreCaseAndProducerId(String name,
            Long producerId);
    ...
    List<Product> findByCategoryIdAndProducerId(
    ...
    List<Product> findByTagIdAndProducerId(@Param("tagId") Long tagId,
            @Param("producerId") Long producerId);

    @Query("SELECT DISTINCT p FROM Product p " + "JOIN p.productCategories pc " + "JOIN p.productTags pt " + "WHERE pc.category.id IN :categoryIds " + "AND pt.tag.id IN :tagIds " + "AND p.producer.id = :producerId")
    List<Product> findByCategoryIdsAndTagIdsAndProducerId(
            @Param("categoryIds") List<Long> categoryIds,
            @Param("tagIds") List<Long> tagIds,
            @Param("producerId") Long producerId);

    @Query("SELECT new pe.com.prueba.plataformacontrolcomercio.dto.producer.ProducerMarketplaceDTO(" + "p.producer.id, p.producer.businessName, p.producer.description, " + "p.producer.location, p.producer.phone, COUNT(p)) " + "FROM Product p " + "WHERE p.producer.approved = true AND p.quantity > 0 " + "GROUP BY p.producer.id, p.producer.businessName, p.producer.description, " + "p.producer.location, p.producer.phone " + "ORDER BY p.producer.businessName")
